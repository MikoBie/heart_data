"""Plot the results from athens: Physical activity"""
...
gdf = (
    athens.loc[athens.iloc[:, 104].apply(lambda x: not isinstance(x, str)), :]
    .groupby(["19 Sex", "version"])
    .agg(
        mean=(
...
gdf = (
    athens.loc[athens.iloc[:, 107].apply(lambda x: not isinstance(x, str)), :]
    .groupby(["19 Sex", "version"])
    .agg(
        mean=(
